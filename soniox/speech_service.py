...
def get_api_key(api_key: Optional[str]) -> str:
    ...
    if api_key == "":
        raise Exception(
            "Soniox API key not specified. Please specify it using the SONIOX_API_KEY "
            "environment variable, by calling soniox.speech_service.set_api_key(), "
            "or by passing the api_key parameter to the SpeechService() constructor."
        )
    return api_key
...
class SpeechClient:
    ...
    def GetTranscribeAsyncStatus(self, file_id: str) -> TranscribeAsyncFileStatus:
